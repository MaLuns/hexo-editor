...
const isPathExist = async (dir: FileSystemDirectoryHandle, path: string): Promise<boolean> => {
    ...
    for (const [index, name] of paths.entries()) {
        if (len === index) {
            if (name !== "") current = await (<FileSystemDirectoryHandle>current)!.getFileHandle(name, { create: false }).catch((_) => null);
        } else {
...
export default class LocalFileSystem extends AbstractFileSystem {
    ...
    async publishPost(path: string): Promise<boolean> {
        ...
        if (sourceFile && postDir) {
            ...
            if (bool && draftDir) {
                const draftAssetsDir = await draftDir.getDirectoryHandle(dirName, { create: false }).catch((_) => null);
                // 如果存在资源文件夹
                if (draftAssetsDir) {
